...
class HomeController extends BaseController
{
    ...
    public function portfolio()
    {
        ...
        if (!empty($cachedata)) {
            ...
            $items = $mainModel->ListsFields('*', 'status = ?', array('1'), 'ordering ASC', 1, 200);
    ...
    public function detail($slug, $id)
    {
        ...
        if (!empty($cachedata)) {
            ...
            if($id == $minId){
                $itemNext = $mainModel->getPostByQuery('projects', 'id, name', 'WHERE id > '.$id.' AND status = 1 ORDER BY id ASC limit 3');
                $linkNext = base_url('/detail/' . create_slug($itemNext['name']) . '/' . $itemNext['id']);

                $itemPrev = $mainModel->getPostByQuery('projects', 'id, name', 'WHERE id = '.$maxId.' AND status = 1');
                $linkPrev = base_url('/detail/' . create_slug($itemPrev['name']) . '/' . $itemPrev['id']);
            } else if($id == $maxId) {
                $itemNext = $mainModel->getPostByQuery('projects', 'id, name', 'WHERE id = '.$minId.' AND status = 1');
                $linkNext = base_url('/detail/' . create_slug($itemNext['name']) . '/' . $itemNext['id']);

                $itemPrev = $mainModel->getPostByQuery('projects', 'id, name', 'WHERE id < '.$id.' AND status = 1 ORDER BY id DESC limit 3');
                $linkPrev = base_url('/detail/' . create_slug($itemPrev['name']) . '/' . $itemPrev['id']);
            } else {
                $itemNext = $mainModel->getPostByQuery('projects', 'id, name', 'WHERE id > '.$id.' AND status = 1 ORDER BY id ASC limit 3');
                $linkNext = base_url('/detail/' . create_slug($itemNext['name']) . '/' . $itemNext['id']);
